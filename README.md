...
```
- do:
	Just like the return statement, just that you don't need a condition and only have one code container that can be executed. You can use the return statement too, to make it return a value.
		Example:
``` 
...
The function name is built up like this:
```
...
Header
Every file is its own namespace. You need to define the properties of the namespace at the Header layer so at the outer layer. This layer will be interpreted in header mode, where you can't use method calls and can only use header statements:
...
- type:
	The type statement is built up like this: type <statement: type>; and can be used, to set the type of the current namespace. Types are:	 
		...
		- internal:
			An internal namespace. You can't create those, but they still exist. It's used for internal functions, like "Console.WriteLine"
...
Keep in mind, that what's been written here might not 100% reflect what's actually true. TASI is a language, that's still in development. We'll try to update the documentation with the latest changes.
